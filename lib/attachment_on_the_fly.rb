  ...
  def generate_image(kind, height = 0, width = 0)
   ...
   # puts self.path
    ...
    if kind == "height"
      ...
      command = "#{convert_command_path}convert -colorspace sRGB -geometry x#{height} -quality 100 -sharpen 1 '#{original}' '#{newfilename}' 2>&1 > /dev/null"
    elsif kind == "width"
      # resize_image infilename, outfilename, width
      command = "#{convert_command_path}convert -colorspace sRGB -geometry #{width} -quality 100 -sharpen 1 '#{original}' '#{newfilename}' 2>&1 > /dev/null"
    elsif kind == "both"
      # resize_image infilename, outfilename, height, width
      command = "#{convert_command_path}convert -colorspace sRGB -geometry #{width}x#{height} -quality 100 -sharpen 1 '#{original}' '#{newfilename}' 2>&1 > /dev/null"
    elsif kind == "#"
      ...
      # crop
      command = "#{convert_command_path}convert -colorspace sRGB -gravity Center -crop #{width}x#{height}+0+0 +repage  -quality 100 '#{newfilename}' '#{newfilename}' 2>&1 > /dev/null"
    end
